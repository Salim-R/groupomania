...
> Une application où les artisans tiennent le carnet de leurs ouvrages, étape par étape. Les visiteurs découvrent le travail en cours, les gestes et le résultat final.
...
```
API          77 tests          0 vulnérabilité en production
...
Se connecter avec `margaux@exemple.fr` et le mot de passe `atelier2026` pour ouvrir un carnet, ajouter des étapes et les réordonner. Tous les comptes de démonstration partagent ce mot de passe ; la liste complète est [plus bas](#comptes-de-démonstration).
